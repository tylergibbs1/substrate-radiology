/**
 * Substrate against the NCI Imaging Data Commons.
 *
 * IDC publishes a public, no-authentication DICOMweb proxy over its whole
 * archive, so the demo reads real de-identified imaging without hosting any.
 * The trade is a per-IP and per-day quota, which is fine for development and is
 * the reason the submission build should point at a self-hosted Orthanc holding
 * only the handful of series the demo actually opens.
 *
 * The seeded case is NLST participant 122615: two annual low-dose chest CT
 * screening rounds a year apart, which is what makes lesion propagation across
 * timepoints demonstrable at all. Study UIDs are in lib/seed.
 *
 * NLST data in IDC is CC BY 4.0. VERIFY the licence on each series before this
 * appears in anything public, and bundle IDC's attribution text with it.
 */
  ...
  dataSources: [
    {
      ...
      configuration: {
        ...
        name: 'idc',
        wadoUriRoot: 'https://proxy.imaging.datacommons.cancer.gov/current/viewer-only-no-downloads-see-tinyurl-dot-com-slash-3j3d9jyp/dicomWeb',
        qidoRoot: 'https://proxy.imaging.datacommons.cancer.gov/current/viewer-only-no-downloads-see-tinyurl-dot-com-slash-3j3d9jyp/dicomWeb',
        wadoRoot: 'https://proxy.imaging.datacommons.cancer.gov/current/viewer-only-no-downloads-see-tinyurl-dot-com-slash-3j3d9jyp/dicomWeb',
